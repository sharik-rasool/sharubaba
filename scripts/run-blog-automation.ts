...
async function updateGoogleSheetStatus(keyword: string, status: string, url: string = "", title: string = "", publishDate: string = "") {
    ...
    try {
        console.log(`[Google Sheet] Sending status update for "${keyword}" to Sheet: Status="${status}", Title="${title}", URL="${url}", PublishDate="${publishDate}"...`);
        const res = await fetch(webhookUrl, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ keyword, status, url, title, publishDate })
        });
        if (!res.ok) {
    ...
    for (let i = 0; i < finalizedPosts.length; i++) {
        ...
        if (dryRun) {
            ...
            try {
                ...
                const formattedDate = post.scheduledFor ? new Date(post.scheduledFor).toISOString().split("T")[0] : new Date().toISOString().split("T")[0];
                await updateGoogleSheetStatus(post.primaryKeyword, "Published", liveUrl, post.title, formattedDate);
            } catch (saveErr: any) {
